...
bool characteristicValueChanged = false;
...
void updateDisplay() { //call this function to go back to the main viewport
  ...
  if (deviceConnected == true) { //check if device is connected to bluetooth 
...
void incrementLastDigit(String& str) {
  // Find the last numerical character and increment it
...
bool connectToBLEServer(const String& deviceID) {
  // Implement BLE server connection logic here
  // Return true if connected successfully, false otherwise
}

void readInitialCharacteristicValue() {
  // Implement reading initial characteristic value logic here
  // Update characteristicValueChanged flag if the value changes
}

void onCharacteristicValueChanged(BLECharacteristic* characteristic) {
  // Callback function when BLE characteristic value changes
...
void setup() {
  ...
  if (bridgeMode) {
    // Increment the last numerical character in deviceID
    incrementLastDigit(deviceID);

    // Connect to BLE server with the new deviceID as SSID
    if (connectToBLEServer(deviceID)) {
      // On first connection, read the characteristic and store the initial value
      readInitialCharacteristicValue();
    }
  }else {
...
void loop() {
  ...
  if ((timeUnconnected == 10000) && (deviceConnected == false)) {
